...
# --- STILIZACIJA (Dugme preko celog ekrana) ---
st.markdown("""
    <style>
    @keyframes blinking {
    ...
    /* Aktivno zeleno dugme (Prijava) */
    .trepcuce-dugme > div > button {
        height: 120px !important; font-size: 26px !important; font-weight: bold !important;
    ...
    /* Onemogućeno sivo dugme (Kada nije izabrano gradilište) */
    .onemoguceno-dugme > div > button {
        height: 120px !important; background-color: #262730 !important;
...
if st.sidebar.text_input("Lozinka:", type="password") == "admin":
    if st.sidebar.checkbox("Prikaži Admin Dashboard"):
        ...
        with tabs[0]: # DANAS
            ...
            if not df_l.empty and broj_r > 0:
        ...
        with tabs[2]: # RADNICI
            if not st.session_state.uredjivanje_cene:
                ...
                if not df_k.empty:
                    ...
                    danas_str, mesec_str = datetime.now().strftime("%d.%m.%Y"), datetime.now().strftime("%m-%Y")
                    ...
                    if 'Cena' in df_k.columns and not df_l.empty:
                        ...
                        if not d_stat.empty:
                            ...
                            for _, row in te_mesec.iterrows(): t_mesec += row['Radni Dani'] * float(c_dict.get(row['Radnik'], 0))
        ...
        with tabs[3]: # TAB DNEVNICE
            if not df_l.empty:
                ...
                if not df_dani_stat.empty:
                    ...
                    prikaz_dnevnica = df_dani_stat[df_dani_stat['Mesec'] == m_izbor]
                    st.table(prikaz_dnevnica[['Radnik', 'Radni Dani']])
...
if not p_ime:
    ...
    if not df_g.empty:
        ...
        if status == "ODLAZAK":
            ...
            # Ako je radnik već prijavljen, pokaži SAMO crveno dugme za odjavu
