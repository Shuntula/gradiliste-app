...
# --- KOLAČIĆI (Mora biti na samom početku) ---
cookies = EncryptedCookieManager(password="neka_veoma_tajna_sifra_123")
if not cookies.ready():
    st.info("Priprema aplikacije... Molimo osvežite stranicu (F5) ako se ništa ne pojavi.")
    st.stop()
...
st.markdown("""
    ...
    .diskretno-dugme { display: flex; justify-content: center; width: 100%; margin-top: 80px !important; }
    .diskretno-dugme > div > button { font-size: 13px !important; color: #888 !important; background-color: transparent !important; border: 1px solid #444 !important; padding: 5px 15px !important; width: auto !important; opacity: 0.7; }
...
def ucitaj_sve_podatke():
    ...
    df_l = get_df("log")
    df_k = get_df("korisnici")
    df_g = get_df("gradilista")
    try: df_t = get_df("troskovi")
...
def azuriraj_cenu_radnika(ime, nova_cena):
    ...
    cell = ws.find(ime)
    if cell: ws.update_cell(cell.row, 3, nova_cena); st.cache_data.clear()
...
try:
    ...
    st.error(f"Baza podataka nije dostupna. Proverite tabove u Google Tabeli. Greška: {e}")
    st.stop()
        ...
        with tabs[1]: # RADNICI
            if not st.session_state.uredjivanje_cene:
                ...
                if not df_k.empty:
                    ...
                    radnici_danas = df_l[(df_l['Akcija'] == 'DOLAZAK') & (df_l['Vreme'].str.contains(danas_dt))]['Radnik'].unique() if not df_l.empty else []
                    t_danas, t_mesec = 0, 0
                    if not df_k.empty and 'Cena' in df_k.columns:
                        ...
                        if not df_stat_dani.empty:
                            te_mesec = df_stat_dani[df_stat_dani['Mesec'] == mesec_dt]
                            for _, row in te_mesec.iterrows(): t_mesec += row['Radni Dani'] * float(cene_dict.get(row['Radnik'], 0))
                    st.markdown(f"<div class='centriran-tekst'><p>Danas: <span class='trosak-box'>{t_danas:,.0f}</span> RSD</p><p>Mesec: <span class='trosak-mesec-box'>{t_mesec:,.0f}</span> RSD</p></div>", unsafe_allow_html=True)
                ...
                cur_c = int(df_k[df_k['Ime'] == iz_r]['Cena'].values[0]) if not df_k.empty else 0
        ...
        with tabs[3]: # DNEVNICE
            if not df_l.empty:
                ...
                if not df_dani_stat.empty:
                    m_izbor = st.selectbox("Mesec:", df_dani_stat['Mesec'].unique())
...
if not p_ime:
    ...
    if st.session_state.unos_troska:
        ...
        l_g = ["-- klikni ovde i izaberi gradilište --"] + df_g['Naziv'].tolist() if not df_g.empty else ["-- nema gradilišta --"]
