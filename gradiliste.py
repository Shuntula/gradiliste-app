...
# --- 3. POVEZIVANJE I UČITAVANJE (POJEDNOSTAVLJENO) ---
def povezi_google():
    try:
        ...
        st.error(f"Problem sa Google autorizacijom: {e}")
        return None
...
def ucitaj_podatke():
    ...
    try:
        ...
        l = pd.DataFrame(sh.worksheet("log").get_all_records())
        k = pd.DataFrame(sh.worksheet("korisnici").get_all_records())
        g = pd.DataFrame(sh.worksheet("gradilista").get_all_records())
        try: t = pd.DataFrame(sh.worksheet("troskovi").get_all_records())
        except: t = pd.DataFrame(columns=["Radnik", "Gradiliste", "Kategorija", "Iznos", "Vreme"])
        return l, k, g, t
    except Exception as e:
        st.error(f"Greška pri čitanju tabele: {e}")
...
def azuriraj_cenu_radnika(ime, nova_cena):
    ...
    if client:
        ...
        cell = ws.find(ime)
        if cell: ws.update_cell(cell.row, 3, nova_cena)
...
# --- 5. POMOĆNE FUNKCIJE ZA ANALITIKU ---
def format_u_hms(sek):
    s, r = divmod(int(sek), 3600)
    m, s = divmod(r, 60)
    return f"{s:02d}:{m:02d}:{s:02d}"
        ...
        if st.sidebar.checkbox("Prikaži Dashboard"):
            ...
            with tabs[1]: # Radnici
                if not st.session_state.get('uredjivanje_cene', False):
                    ...
                    # Brzi obračun troškova
                    d_s = datetime.now().strftime("%d.%m.%Y")
                    r_d = df_l[(df_l['Akcija'] == 'DOLAZAK') & (df_l['Vreme'].str.contains(d_s))]['Radnik'].unique() if not df_l.empty else []
                    t_d = df_k[df_k['Ime'].isin(r_d)]['Cena'].astype(float).sum() if 'Cena' in df_k.columns else 0
                    st.markdown(f"<div class='centriran-tekst'><span class='trosak-box'>Danas: {t_d:,.0f} RSD</span></div>", unsafe_allow_html=True)
                    ...
                    if st.button("Sačuvaj"): azuriraj_cenu_radnika(r_sel, n_c); st.session_state.uredjivanje_cene = False; st.rerun()
            ...
            with tabs[4]: # Gradilista
                n_g = st.text_input("Novo gradilište:")
                if st.button("Dodaj"): dodaj_u_tabelu("gradilista", [n_g]); st.rerun()
                st.dataframe(df_g, use_container_width=True)

            with tabs[5]: # Troškovi
                if not df_t.empty:
                    st.dataframe(df_t.iloc[::-1], use_container_width=True)
                    st.metric("Ukupno", f"{df_t['Iznos'].astype(float).sum():,.0f}")
            st.stop()
            ...
            if status == "ODLAZAK":
                if izbor == "-- klikni ovde i izaberi gradilište --":
                    ...
                    if st.button("✅ PRIJAVI SE NA POSAO"):
                        dodaj_u_tabelu("log", [p_ime, "DOLAZAK", izbor, v_sad]); st.rerun()
                    st.markdown('</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="odjava-dugme">', unsafe_allow_html=True)
                if st.button("🛑 ODJAVI SE SA POSLA"):
                    dodaj_u_tabelu("log", [p_ime, "ODLAZAK", izbor, v_sad]); st.rerun()
                st.markdown('</div>', unsafe_allow_html=True)
