...
def ucitaj_sve_podatke():
    ...
    try:
        df_t = pd.DataFrame(sh.worksheet("troskovi").get_all_records())
    except:
        df_t = pd.DataFrame(columns=["Radnik", "Gradiliste", "Kategorija", "Iznos", "Vreme"])
...
if st.sidebar.text_input("Lozinka:", type="password") == "admin":
    if st.sidebar.checkbox("Prikaži Admin Dashboard"):
        ...
        with tabs[5]: # TAB TROŠKOVI (ADMIN)
            st.subheader("Evidencija dodatnih troškova")
            if not df_t.empty:
                st.dataframe(df_t.iloc[::-1], use_container_width=True)
                st.metric("Ukupno svi troškovi", f"{df_t['Iznos'].astype(float).sum():,.0f} RSD")
            else: st.info("Nema unetih troškova.")
        ...
        with tabs[2]: # RADNICI
            if not st.session_state.uredjivanje_cene:
                ...
                if not df_k.empty:
                    ...
                    # Proračun troškova (isto kao pre)
                ...
                new_c = st.number_input("Cena:", step=100)
...
if not p_ime:
    ...
    if st.session_state.unos_troska:
        # STRANICA ZA UNOS TROŠKA
        st.subheader("💰 Unos novog troška")
        if st.button("⬅️ Otkaži"): st.session_state.unos_troska = False; st.rerun()
        
        kategorija = st.selectbox("Šta ste platili?", ["GORIVO", "HRANA", "MATERIJAL", "DRUGO"])
        iznos = st.number_input("Iznos u RSD:", min_value=0, step=50)
        grad_trosak = st.selectbox("Za koje gradilište?", df_g['Naziv'].tolist() if not df_g.empty else ["Nema"])
        
        if st.button("✅ SAČUVAJ TROŠAK", use_container_width=True):
            if iznos > 0:
                vreme_t = datetime.now().strftime("%d.%m.%Y %H:%M:%S")
                dodaj_u_tabelu("troskovi", [p_ime, grad_trosak, kategorija, iznos, vreme_t])
                st.success("Trošak uspešno zabeležen!")
                st.session_state.unos_troska = False
                st.rerun()
    else:
        # GLAVNI EKRAN
        ...
        if not df_l.empty:
            ...
            if not r_logs.empty:
                status = r_logs.iloc[-1]['Akcija']; posl_g = r_logs.iloc[-1]['Gradiliste']
